...
mv_data_dir = '/home/hamid/Desktop/OpticalFlow/FlowSciVis/Datasets'
'''  
    KITTI flow data:
...
class kitti_train:
    ...
    # def mv_data_get_file_names(cls, if_test=True):
        print("mv_data_get_file_names")
        ...
        if os.path.isfile(file_names_save_path) and not if_test:
            ...
            if os.path.isdir(mv_2015_dir):
                print("os.path.isdir(mv_2015_dir)")
                pass
            else:
    ...
    class kitti_data_with_start_point(Dataset):
        class config(tools.abstract_config):
            ...
            def __init__(self, **kwargs):
                print("kitti_data_with_start_point.config")
            ...
            def __call__(self):
                print("kitti_data_with_start_point.config.__call__")
                return kitti_train.kitti_data_with_start_point(self)

        def __init__(self, conf: config):
            self.conf = conf
            print("kitti_data_with_start_point.__init__")
            print("self.conf.mv_type", self.conf.mv_type)
            if self.conf.mv_type in ['2015', '2012']:
                print("true")
                file_dict_ = kitti_train.mv_data_get_file_names()
                print("true x")
                # print("file_dict_", file_dict_)
                # self.conf.mv_type = '2012'
...
class kitti_flow:
    class Evaluation_bench():
        ...
        def __init__(self, name, if_gpu=True, batch_size=1):
            ...
            input("self.dataset kitti")
            self.loader = tools.data_prefetcher(self.dataset, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True, drop_last=False)
