    ...
    class kitti_data_with_start_point(Dataset):
        ...
        def __init__(self, conf: config):
            ...
            if self.conf.mv_type in ['2012']:
                ...
                raise ValueError('mv_type should be 2012')
            # =====================================
...
class kitti_flow:
    class Evaluation_bench():
        ...
        def __init__(self, name, if_gpu=True, batch_size=1):
            ...
            print(name)
            assert name in ['2012_train', '2012_test']
            self.name = name
    ...
    def get_file_names(cls, if_test=False):
        ...
        file_names_save_path = os.path.join(kitti_flow_dir, 'kitti_flow_2012_file_names.pkl')
        # if os.path.isfile(file_names_save_path) and not if_test:
        if 0 == 1:
            data = tools.pickle_saver.load_picke(file_names_save_path)
            return data
    ...
    class kitti_train():
        ...
        def __init__(self, name):
            assert name in ['2012_train', '2012_test']  # ['2012_train', '2015_train']
            data = kitti_flow.get_file_names()
            print("in kitti_train __init__")
            # print(data)
            # input("data")
            print("name", name)
        ...
        def demo(cls):
            ...
            data = kitti_flow.kitti_train('2012_test')
