...
class UPFlow_net(tools.abstract_model):
    ...
    def scivis_data_dict(self, data_tensor): # img0, img1, gt
        ...
        for i in range(data_tensor.shape[0]):
            im1_np = data_tensor[i, 0]
            im2_np = data_tensor[i, 1]
            # print(im1_np.shape, im2_np.shape)

            # to rgb
            im1_np = np.stack((im1_np, im1_np, im1_np), axis=0)
            # im1_np = np.concatenate((im1_np, im1_np, im1_np), axis=0)
            im2_np = np.stack((im2_np, im2_np, im2_np), axis=0)
    ...
    def kitti_inference_data_dict(self, batch): # img0, img1, gt
        # print("batch", batch)

        batch['im1'] = torch.Tensor(np.array(batch['im1'])).to(device)
        batch['im2'] = torch.Tensor(np.array(batch['im2'])).to(device)
    ...
    def forward(self, input_dict: dict):
        ...
        input_dict = self.kitti_data_dict(input_dict) # training kitti
        # input_dict = self.kitti_inference_data_dict(input_dict) # inference kitti
        # input("forward")
