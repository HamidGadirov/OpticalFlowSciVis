...
from error import create_data_for_interpol

warnings.filterwarnings("ignore")

from matplotlib import pyplot as plt

def visualize_ind(image, name="result.png", save=False):
    fig = plt.figure()
    ax = fig.gca()
...
def visualize_series(data_to_vis, title="Data", show=True, save=False):
    fig=plt.figure()
    columns = 10
    rows = 6
...
while True:
    ...
    if ssim > 0.996:        
        ...
        I1 = torch.from_numpy(np.transpose(frame, (2,0,1))).to(device, non_blocking=True).unsqueeze(0).float() / 255.
        I1 = pad_image(I1)
        I1, flow = model.inference(I0, I1, args.scale) # get the flow too
        I1_small = F.interpolate(I1, (32, 32), mode='bilinear', align_corners=False)
        ssim = ssim_matlab(I0_small[:, :3], I1_small[:, :3])
...
if args.png == False and fpsNotAssigned == True and not args.video is None:
    try:
        ...
        print(flow_combined.shape) # (294, 4, 160, 224)
        ...
        visualize_series(bgr, title="FlowHSV_01", show=True, save=True)
        ...
        visualize_series(interpol_data, title="Interpolated timesteps", show=True, save=True)
