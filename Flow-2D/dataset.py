...
class VimeoDataset(Dataset):
    ...
    def load_data(self):
        ...
        if self.dataset_name == 'train':
            ...
            print("train", len(self.meta_data))
        elif self.dataset_name == 'test':
            self.meta_data = self.testlist
            self.meta_data = self.meta_data[:128] # div to 16
            print("test", len(self.meta_data))
        else:
            self.meta_data = self.trainlist[cnt:]
            self.meta_data = self.meta_data[:32] # div to 16
            print("val", len(self.meta_data))
